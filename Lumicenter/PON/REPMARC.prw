...
Local nCont
Local nQtdHoras := GetMV("LC_QTDHORA")
PRIVATE lMsErroAuto := .F.

cQuery :=" select  P8_FILIAL, P8_MAT, P8_DATAAPO, MAX(RA_REGRA) REGRA, MAX(RA_TNOTRAB) TURNO, Max(RA_CC) RA_CC from " + RetSqlName("SP8") +" SP8 " + cLFRC
...
cQuery +=" and P8_DATAAPO >= '"+ DtoS(MV_PAR07) + "' and P8_DATAAPO <= '"+ DtoS(MV_PAR08)+"'"
...
While QRY->(! EOF())
	...
	if SPJ->( DbSeek( QRY->( P8_FILIAL + TURNO) + '01' + cValToChar(nDia)))
		if (SP8->P8_HORA <> SPJ->PJ_SAIDA1) 
			if (SP8->( dbseek(QRY->(P8_FILIAL + P8_MAT+ P8_DATAAPO + '1S') )))
				if (SP8->P8_HORA - SPJ->PJ_SAIDA1)  < (nQtdHoras * -1) .oR.  SP8->P8_HORA - SPJ->PJ_SAIDA1 > nQtdHoras
					gravaSP8(cOrdem, cPaPonta)
				endif
			elseif (SP8->( dbseek(QRY->(P8_FILIAL + P8_MAT+ P8_DATAAPO + '2E') )))
				if (SP8->P8_HORA - SPJ->PJ_SAIDA1)  < (nQtdHoras * -1) .oR.  SP8->P8_HORA - SPJ->PJ_SAIDA1 > nQtdHoras
					gravaSP8(cOrdem, cPaPonta)
				endif
			elseif (SP8->( dbseek(QRY->(P8_FILIAL + P8_MAT+ P8_DATAAPO ) )))
				gravaSP8(cOrdem, cPaPonta)
...
Static function gravaSP8(cOrdem, cPaPonta)

	// verificar se horário 
