   ...
   If _iOpcao == 2 .Or. _iOpcao == 3//Incluir ou Alterar
		...
		Begin transaction 
			...
			if(_iOpcao == 2) // Incluir
				aCampos:={}
				Aadd(aCampos,{"Z39_FILIAL"  ,"", xFilial("Z39")})
				Aadd(aCampos,{"Z39_NUM"     ,"", _cNum      })
				Aadd(aCampos,{"Z39_VIGENC"  ,"", _cVigencia })
				Aadd(aCampos,{"Z39_FUNCIO"  ,"", _cFuncional})
				Aadd(aCampos,{"Z39_DTINIC"  ,Stod('  /  /  '), _dDtIni    })
				Aadd(aCampos,{"Z39_DATAFI"  ,Stod('  /  /  '), _dDtFim    })
				...
				if RecLock("Z39",.T.)
					...
					u_GravaLog(_iOpcao, aCampos,"F")			
				MsUnLock("Z39")
					ConfirmSX8()

					// chamar a funcao para gravacao log passando o usuario- RetCodUsr( )
				else
					RollbackSx8()
				endif
			elseif(_iOpcao == 3) //

			//Alterar a SZ3 quando mudar o Status

			if empty(_dDtFim)
			_cVigencia := "S"
			endif
				if _cVigencia <> Z39->Z39_VIGENC
					aCampos:={}
					Aadd(aCampos,{"Z39_VIGENC",Z39->Z39_VIGENC,_cVigencia})
					Aadd(aCampos,{"Z39_DATAFI",Z39->Z39_DATAFI,_dDtFim})
				...
				While !Z40->(Eof()) .And. Z40->( Z40_FILIAL + Z40_NUM) == Z39->( Z39_FILIAL + Z39_NUM)
					Aadd(aCamposZ40, {Z40->Z40_SEQ,Z40->Z40_CC,Z40->Z40_PERCRA,Z40->Z40_DIRECI,Z40->Z40_ATIVAC})
					RecLock("Z40",.F.)
						Z40->(DbDelete())
					MsUnLock("Z40")
					Z40->( Dbskip() )
				...
				if _iOpcao == 3 // Alterar
					aCampos:= u_CompArrays(aCamposZ40, aCols)
					if len(aCampos) >0
						u_GravaLog(_iOpcao, aCampos,"F")
					Endif
				Endif
			...
			For _iCount := 1 To Len(aCols)
				If !aCols[_iCount, Len(aCols[1])] //Se nao estiver deletado
					if(_iOpcao == 2) // incluir
						aCampos:={}
						Aadd(aCampos,{"Z40_FILIAL","",xFilial("Z40")})
						Aadd(aCampos,{"Z40_NUM",   "",_cNum})
						Aadd(aCampos,{"Z40_FUNCIO","",_cFuncional})
						Aadd(aCampos,{"Z40_CC",    "",aCols[_iCount, _iPosCC]})
						Aadd(aCampos,{"Z40_PERCRA",0,aCols[_iCount, _iPosPercRa]})
						Aadd(aCampos,{"Z40_DIRECI","",aCols[_iCount, _iPosDirec]})
						Aadd(aCampos,{"Z40_ATIVAC","",aCols[_iCount, _iPosAtiva]})
						u_GravaLog(_iOpcao, aCampos,"F")
					endif
	...
	ElseIf _iOpcao == 4 //Excluir
		...
		BEGIN TRANSACTION
			...
			While !Z39->(Eof()) .And.  Z39->( Z39_FILIAL + Z39_NUM) == cChave
				aCampos:={}
				Aadd(aCampos,{"Z39_FILIAL",Z39->Z39_FILIAL,""})
				Aadd(aCampos,{"Z39_FUNCIO",Z39->Z39_FUNCIO,""})
				Aadd(aCampos,{"Z39_NUM",Z39->Z39_NUM,""})
				Aadd(aCampos,{"Z39_VIGENC",Z39->Z39_VIGENC,""})
				Aadd(aCampos,{"Z39_DTINIC",Z39->Z39_DTINIC,Stod(' /  /  ')})
				Aadd(aCampos,{"Z39_DATAFI",Z39->Z39_DATAFI,Stod(' /  /  ')})
			...
			While !Z40->(Eof()) .And. Z40->( Z40_FILIAL + Z40_NUM) == cChave
				aCampos:={}
				Aadd(aCampos,{"Z40_FILIAL",Z40->Z40_FILIAL,""})
				Aadd(aCampos,{"Z40_NUM",   Z40->Z40_NUM,""})
				Aadd(aCampos,{"Z40_FUNCIO",Z40->Z40_FUNCIO,""})
				Aadd(aCampos,{"Z40_CC",    Z40->Z40_CC,""})
				Aadd(aCampos,{"Z40_PERCRA",Z40->Z40_PERCRA,0})
				Aadd(aCampos,{"Z40_DIRECI",Z40->Z40_DIRECI,""})
				Aadd(aCampos,{"Z40_ATIVAC",Z40->Z40_ATIVAC,""})					
...
Static Function MenuDef()
	Local aRotina := { {"Pesquisar       " , "AxPesqui"         , 0 , 1},;
                		 ...
                		 {"Anexar Doc      " , "U_AnexRat("+Z39->Z39_NUM+")"   , 0 , 5}}
Return(aRotina)
