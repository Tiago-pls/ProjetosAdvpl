	...
	While !CNB->( Eof() ) .And. CNB->(CNB_FILIAL+CNB_CONTRA+CNB_REVISA+CNB_NUMERO) == CNA->(CNA_FILIAL+CNA_CONTRA+CNA_REVISA+CNA_NUMERO)
		...
		IF CNB->CNB_SUBST  != 'S' .And. Empty(CNB->CNB_ITMDST) .and. ( Empty(aEquip) .or. aScan(aEquip,CNB->CNB_ITEM) >0)
			...
			//If cEmpAnt = '09'
				...
				//OLE_SetDocumentVar( oWord, 'ITEM3' + cValToChar(nLinha), CNB->CNB_ITEM )
				//OLE_SetDocumentVar( oWord, 'ITEM4' + cValToChar(nLinha), CNB->CNB_DESCRI )
				//OLE_SetDocumentVar( oWord, 'ITEM5' + cValToChar(nLinha), alltrim(transform(CNB->CNB_VLUNIT,PesqPict("CNB","CNB_VLUNIT"))))
/*
			Else
			
				cItensDesc += CNB->CNB_ITEM + ") " + alltrim(CNB->CNB_PRODUT) + " - " + alltrim(IIF(SB5->(Found()),SB5->B5_CEME,CNB->CNB_DESCRI))
				IF CNB->(FieldPos("CNB_XHORIM")) > 0 .And. CNB->CNB_XHORIM>0
					cItensDesc += " - HORÍMETRO " + cValToChar(CNB->CNB_XHORIM)
				EndIF
				cItensDesc += ";" + CRLF+CRLF
				cItensCond += CNB->CNB_ITEM + ") " + "Para o equipamento descrito na cláusula 1.1. '" + CNB->CNB_ITEM + "', fica estipulado o valor de R$ "+alltrim(transform(CNB->CNB_VLUNIT,PesqPict("CNB","CNB_VLUNIT")))+" ("+Capital(Extenso(CNB->CNB_VLUNIT,.F.,CN9->CN9_MOEDA,,"1",.T.,.F.))+") por mês de locação;" + CRLF+CRLF
				cItensCDia += CNB->CNB_ITEM + ") " + "Para o equipamento descrito na cláusula 1.1., '" + CNB->CNB_ITEM + "', fica estipulado o valor de R$ "+alltrim(transform(CNB->CNB_VLUNIT*8,PesqPict("CNB","CNB_VLUNIT")))+" ("+Capital(Extenso(CNB->CNB_VLUNIT*8,.F.,CN9->CN9_MOEDA,,"1",.T.,.F.))+") por dia de locação, totalizando R$ "+alltrim(transform(CNB->CNB_VLTOT,PesqPict("CNB","CNB_VLTOT")))+" ("+Capital(Extenso(CNB->CNB_VLTOT,.F.,CN9->CN9_MOEDA,,"1",.T.,.F.))+") pela locação;" + CRLF+CRLF
				cItensExce += CNB->CNB_ITEM + ") " + "Para o equipamento descrito na cláusula 1.1., '" + CNB->CNB_ITEM + "', fica estipulado o valor de R$ "+alltrim(transform(CNB->CNB_VLUNIT,PesqPict("CNB","CNB_VLUNIT")))+" ("+Capital(Extenso(CNB->CNB_VLUNIT,.F.,CN9->CN9_MOEDA,,"1",.T.,.F.))+") por hora trabalhada que exceder o regime de oito horas diárias, conforme horímetro do(s) equipamento(s);" + CRLF+CRLF
	...
	//If cEmpAnt = '09'
		...
		//OLE_ExecuteMacro( oWord, "tabPrc")

	/*Else

		OLE_SetDocumentVar( oWord, 'QTDITENS', cValToChar(nLinha) )
		OLE_SetDocumentVar( oWord, 'cItensDescricao', cItensDesc )
		OLE_SetDocumentVar( oWord, 'cItensClausula', cItensCond )
		OLE_SetDocumentVar( oWord, 'cItensClausulaDia', cItensCDia )
		OLE_SetDocumentVar( oWord, 'cItensExcesso', cItensExce )
 
	EndIf

	OLE_SetDocumentVar( oWord, 'ValorUniTotal', alltrim(transform(nValorUniTotal,PesqPict("CNB","CNB_VLUNIT"))))
	OLE_SetDocumentVar( oWord, 'ValorUniTotalExtenso', Capital(Extenso(nValorUniTotal,.F.,CN9->CN9_MOEDA,,"1",.T.,.F.)) )

	OLE_SetDocumentVar( oWord, 'ValorTotal', alltrim(transform(nValorTotal,PesqPict("CNB","CNB_VLUNIT"))))
	OLE_SetDocumentVar( oWord, 'ValorTotalExtenso', Capital(Extenso(nValorTotal,.F.,CN9->CN9_MOEDA,,"1",.T.,.F.)) )
*/
